...
export default function Pilibangan() {
    // const context = useContext(noteContext);
    // const {addUser} = context;
    const [user,setUser] = useState({name:"",mobileno:"",address:"Pilibangan"});
    const [newuser,setNewuser] = useState({name:"",mobileno:"",address:"Pilibangan"});
    ...
    const handleClick = (e)=>{
        ...
        setNewuser({name:"",mobileno:"",address:"Pilibangan"})
    }
    ...
    const getUser = async () => {
        try {
            const response = await fetch(`http://localhost:8080/user/getbycity?address=Pilibangan`, {
                method: 'GET'
            });
  ...
  return (
    <div>
        <div className='container border rounded my-2'>
            <h1>Add a User in Pilibangan Account</h1>
